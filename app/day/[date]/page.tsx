  ...
  return (
    <Flex direction="column" minH="calc(100dvh - var(--tabbar-h))">
      <Box px={{ base: '20px', lg: '28px' }} pt="24px" pb="16px">
        ...
        {day.highlight && (
          <Box className="only-mobile" fontSize="12px" color="mario.dim" mt="10px">
            重點:{day.highlight}
          </Box>
        )}
      </Box>
      <Box className="only-mobile" borderTopWidth="1px" borderColor="mario.line" />
...
function DayRail({ day, subtotal }: { day: TripDay; subtotal: number }): React.JSX.Element {
  const todos = day.items.filter((i) => i.reservationStatus === '待預約' || i.reservationStatus === '候補中');
  return (
    <aside className="day-rail only-desktop">
      <Box className="panel-desktop" p="16px">
        ...
        <RailKv k="預估花費">
          <Box as="span" fontFamily="mono" fontSize="15px" fontWeight="600" color="mario.yellow">
            {formatYen(subtotal)}
          </Box>
        </RailKv>
      </Box>
      ...
      {(todos.length > 0 || day.highlight) && (
        <Box className="panel-desktop" p="16px">
          ...
          {todos.map((t) => (
            <Flex key={t.name} fontSize="12.5px" mt="10px" justify="space-between" align="center" gap="10px">
              <span>{t.name}</span>
